...
static_assert(MLKEM768Key::entropy_size == MLKEM768_KEYGEN_SEED_BYTES,
              "cuda_pqc.h keygen seed size must match cuPQC entropy size");
static_assert(MLKEM768Encaps::entropy_size == MLKEM768_ENCAPS_SEED_BYTES,
              "cuda_pqc.h encaps seed size must match cuPQC entropy size");

template <typename OperatorT>
static cudaError_t copy_entropy_to_device(uint8_t *device_entropy,
                                          const uint8_t *host_seeds,
                                          size_t count)
{
    if (!device_entropy || !host_seeds || count == 0)
        return cudaErrorInvalidValue;
    const size_t total = OperatorT::entropy_size * count;
    return cudaMemcpy(device_entropy, host_seeds, total,
                                               ...
                                               size_t count)
{
    ...
    rc = copy_entropy_to_device<MLKEM768Key>(randombytes, seeds, count);
    if (rc != cudaSuccess) goto fail;
...
extern "C" int skim_cuda_mlkem768_encaps_batch(const uint8_t *pk,
                                               ...
                                               size_t count)
{
    if (!skim_cuda_pqc_available() || count == 0 || !pk || !seeds ||
        !ct || !ss)
        return -1;
    ...
    rc = cudaMemcpy(d_pk, pk, pk_total, cudaMemcpyHostToDevice);
    if (rc != cudaSuccess) goto fail;
    rc = copy_entropy_to_device<MLKEM768Encaps>(randombytes, seeds, count);
    if (rc != cudaSuccess) goto fail;
...
extern "C" pqc_bench_result_t skim_cuda_mlkem768_bench_keygen(size_t count)
{
    ...
    uint8_t *seeds = (uint8_t *)calloc(MLKEM768_KEYGEN_SEED_BYTES * count, 1);
