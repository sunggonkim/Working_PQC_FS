/**
 * pqc_admission.c — QoS-aware elastic-lane admission
 *
 * This module implements the mounted runtime's conservative admission boundary
 * for GPU-eligible maintenance work.  Foreground AES-GCM publication remains
 * CPU-first before this controller; admission only decides whether independent,
 * batched, slack-tolerant key-plane work may enter the elastic GPU lane.
 *
 * Missing or stale producer slack, insufficient budget, deadline violations,
 * small batches, and queue/memory-pressure signals fail closed to CPU.  The
 * telemetry inputs are software/runtime signals, not a claim of privileged
 * scheduler control or hardware counter completeness.
 *
...
#define PQC_ADMISSION_DEFAULT_GPU_MIN_BATCH_BYTES 131072ULL
...
#define PQC_ADMISSION_TELEMETRY_SCALE 10000U
...
static atomic_int g_admission_trace_enabled = ATOMIC_VAR_INIT(0);

static atomic_uint g_telemetry_mem_bandwidth_scaled = ATOMIC_VAR_INIT(0);
static atomic_uint g_telemetry_tensor_core_scaled = ATOMIC_VAR_INIT(0);
...
static struct {
    atomic_uint_fast64_t gpu_admitted_count;
    atomic_uint_fast64_t cpu_routed_count;
    atomic_uint_fast64_t ai_budget_exhausted_count;
    atomic_uint_fast64_t coherence_risk_count;
    atomic_uint_fast64_t queue_pressure_count;
    atomic_uint_fast64_t deadline_exceeded_count;
    atomic_uint_fast64_t size_too_small_count;
    atomic_uint_fast64_t stale_telemetry_count;
    atomic_uint_fast64_t total_requests;
} g_admission_stats_atomic;

static void admission_stats_snapshot(pqc_admission_stats_t *out_stats)
{
    if (!out_stats)
        return;
    out_stats->gpu_admitted_count =
        atomic_load_explicit(&g_admission_stats_atomic.gpu_admitted_count,
                             memory_order_relaxed);
    out_stats->cpu_routed_count =
        atomic_load_explicit(&g_admission_stats_atomic.cpu_routed_count,
                             memory_order_relaxed);
    out_stats->ai_budget_exhausted_count =
        atomic_load_explicit(&g_admission_stats_atomic.ai_budget_exhausted_count,
                             memory_order_relaxed);
    out_stats->coherence_risk_count =
        atomic_load_explicit(&g_admission_stats_atomic.coherence_risk_count,
                             memory_order_relaxed);
    out_stats->queue_pressure_count =
        atomic_load_explicit(&g_admission_stats_atomic.queue_pressure_count,
                             memory_order_relaxed);
    out_stats->deadline_exceeded_count =
        atomic_load_explicit(&g_admission_stats_atomic.deadline_exceeded_count,
                             memory_order_relaxed);
    out_stats->size_too_small_count =
        atomic_load_explicit(&g_admission_stats_atomic.size_too_small_count,
                             memory_order_relaxed);
    out_stats->stale_telemetry_count =
        atomic_load_explicit(&g_admission_stats_atomic.stale_telemetry_count,
                             memory_order_relaxed);
    out_stats->total_requests =
        atomic_load_explicit(&g_admission_stats_atomic.total_requests,
                             memory_order_relaxed);
}

static void admission_stats_restore(const pqc_admission_stats_t *stats)
{
    if (!stats)
        return;
    atomic_store_explicit(&g_admission_stats_atomic.gpu_admitted_count,
                          stats->gpu_admitted_count, memory_order_relaxed);
    atomic_store_explicit(&g_admission_stats_atomic.cpu_routed_count,
                          stats->cpu_routed_count, memory_order_relaxed);
    atomic_store_explicit(&g_admission_stats_atomic.ai_budget_exhausted_count,
                          stats->ai_budget_exhausted_count,
                          memory_order_relaxed);
    atomic_store_explicit(&g_admission_stats_atomic.coherence_risk_count,
                          stats->coherence_risk_count, memory_order_relaxed);
    atomic_store_explicit(&g_admission_stats_atomic.queue_pressure_count,
                          stats->queue_pressure_count, memory_order_relaxed);
    atomic_store_explicit(&g_admission_stats_atomic.deadline_exceeded_count,
                          stats->deadline_exceeded_count,
                          memory_order_relaxed);
    atomic_store_explicit(&g_admission_stats_atomic.size_too_small_count,
                          stats->size_too_small_count, memory_order_relaxed);
    atomic_store_explicit(&g_admission_stats_atomic.stale_telemetry_count,
                          stats->stale_telemetry_count, memory_order_relaxed);
    atomic_store_explicit(&g_admission_stats_atomic.total_requests,
                          stats->total_requests, memory_order_relaxed);
}

static void admission_stats_reset(void)
{
    const pqc_admission_stats_t zero = {0};
    admission_stats_restore(&zero);
}

static void admission_stats_record(unsigned flags)
{
    atomic_fetch_add_explicit(&g_admission_stats_atomic.total_requests, 1,
                              memory_order_relaxed);
    if (flags & ADMISSION_STATS_GPU_ADMITTED)
        atomic_fetch_add_explicit(&g_admission_stats_atomic.gpu_admitted_count,
                                  1, memory_order_relaxed);
    if (flags & ADMISSION_STATS_CPU_ROUTED)
        atomic_fetch_add_explicit(&g_admission_stats_atomic.cpu_routed_count,
                                  1, memory_order_relaxed);
    if (flags & ADMISSION_STATS_AI_BUDGET_EXHAUSTED)
        atomic_fetch_add_explicit(
            &g_admission_stats_atomic.ai_budget_exhausted_count, 1,
            memory_order_relaxed);
    if (flags & ADMISSION_STATS_COHERENCE_RISK)
        atomic_fetch_add_explicit(&g_admission_stats_atomic.coherence_risk_count,
                                  1, memory_order_relaxed);
    if (flags & ADMISSION_STATS_QUEUE_PRESSURE)
        atomic_fetch_add_explicit(&g_admission_stats_atomic.queue_pressure_count,
                                  1, memory_order_relaxed);
    if (flags & ADMISSION_STATS_DEADLINE_EXCEEDED)
        atomic_fetch_add_explicit(
            &g_admission_stats_atomic.deadline_exceeded_count, 1,
            memory_order_relaxed);
    if (flags & ADMISSION_STATS_SIZE_TOO_SMALL)
        atomic_fetch_add_explicit(&g_admission_stats_atomic.size_too_small_count,
                                  1, memory_order_relaxed);
    if (flags & ADMISSION_STATS_STALE_TELEMETRY)
        atomic_fetch_add_explicit(
            &g_admission_stats_atomic.stale_telemetry_count, 1,
            memory_order_relaxed);
}

static unsigned admission_telemetry_to_scaled(double value)
{
    if (value <= 0.0)
        return 0U;
    if (value >= 1.0)
        return PQC_ADMISSION_TELEMETRY_SCALE;
    return (unsigned)(value * (double)PQC_ADMISSION_TELEMETRY_SCALE + 0.5);
}

static double admission_telemetry_from_scaled(unsigned value)
{
    if (value > PQC_ADMISSION_TELEMETRY_SCALE)
        value = PQC_ADMISSION_TELEMETRY_SCALE;
    return (double)value / (double)PQC_ADMISSION_TELEMETRY_SCALE;
}

static double admission_telemetry_mem_read(void)
{
    return admission_telemetry_from_scaled(atomic_load_explicit(
        &g_telemetry_mem_bandwidth_scaled, memory_order_relaxed));
}

static double admission_telemetry_tensor_read(void)
{
    return admission_telemetry_from_scaled(atomic_load_explicit(
        &g_telemetry_tensor_core_scaled, memory_order_relaxed));
}

static void admission_telemetry_write(double mem_bandwidth_util,
                                      double tensor_core_util)
{
    atomic_store_explicit(&g_telemetry_mem_bandwidth_scaled,
                          admission_telemetry_to_scaled(mem_bandwidth_util),
                          memory_order_relaxed);
    atomic_store_explicit(&g_telemetry_tensor_core_scaled,
                          admission_telemetry_to_scaled(tensor_core_util),
                          memory_order_relaxed);
}
...
static void admission_state_snapshot_locked(admission_state_snapshot_t *snapshot)
{
    ...
    admission_stats_snapshot(&snapshot->stats);
    ...
    snapshot->telemetry_mem_bandwidth_util = admission_telemetry_mem_read();
    snapshot->telemetry_tensor_core_util = admission_telemetry_tensor_read();
}
...
static void admission_state_restore_locked(const admission_state_snapshot_t *snapshot)
{
    ...
    admission_stats_restore(&snapshot->stats);
    ...
    admission_telemetry_write(snapshot->telemetry_mem_bandwidth_util,
                              snapshot->telemetry_tensor_core_util);
}
...
static int admission_trace_enabled_fast(void)
{
    return atomic_load_explicit(&g_admission_trace_enabled,
...
static void admission_trace_append(const char *line)
{
    if (!line)
        return;
    if (!admission_trace_enabled_fast())
        return;
...
int pqc_admission_init(const char *trace_file_path)
{
    ...
    admission_stats_reset();
    admission_telemetry_write(telemetry_mem_bandwidth,
                              telemetry_tensor_core);
    ...
    atomic_store_explicit(&g_admission_trace_enabled, trace_fd >= 0,
...
void pqc_admission_shutdown(void)
{
    ...
    admission_stats_snapshot(&stats_snapshot);
    ...
    if (admission_trace_lock(&trace_scope, "admission_shutdown_trace") == 0) {
        ...
        atomic_store_explicit(&g_admission_trace_enabled, 0,
                              memory_order_release);
...
int pqc_admit(pqc_admission_context_t *ctx)
{
    ...
    /* ─────────────────────────────────────────────────────────────────────
     * Step 0: Snapshot policy under lock; telemetry is atomic.
     * ───────────────────────────────────────────────────────────────────── */
    ...
    double mem_bw = admission_telemetry_mem_read();
    double tc_util = admission_telemetry_tensor_read();
...
void pqc_scheduler_trace_log(const pqc_admission_context_t *ctx)
{
    ...
    scheduler_trace_log_snapshot(ctx, admission_telemetry_mem_read(),
                                 admission_telemetry_tensor_read());
}
...
void pqc_scheduler_trace_stats(pqc_admission_stats_t *out_stats)
{
    ...
    admission_stats_snapshot(out_stats);
}
...
{
    admission_telemetry_write(mem_bandwidth_util, tensor_core_util);
}
