...
def run_freshness_window_model():
    print("Running analytical freshness-window model (not a power-loss test)...")
    ...
    results = {
        ...
        "not_power_loss_evidence": True,
        "claim_boundary": (
            "This models freshness-window size versus recovery work. It is not "
            "a physical power-loss, VM power-cut, kernel-crash, drive-cache, or "
            "mounted lower-block interruption campaign."
        ),
    ...
    print("\n[+] Freshness-window model completed.")
    print("[+] Boundary: analytical model only; not physical power-loss, VM power-cut, kernel-crash, or drive-cache evidence.")

if __name__ == "__main__":
    run_freshness_window_model()
