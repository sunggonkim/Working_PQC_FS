...
FUSE_SOURCE = CODE / "frontend" / "pqc_fuse.c"
TRACE_SCRIPT = CODE / "experiments" / "trace_nvme_lat.bt"
TRACE_OUT = ROOT / "artifacts" / "probes" / "evidence" / "trace_nvme_lat.out"
IOURING_PROBE = CODE / "experiments" / "bench_io_uring_ebpf.cu"
...
REQUIRED_PAPER_PHRASES = [
    ...
    "Mounted eBPF/io\\_uring path",
    "No eBPF notification path is part of the mounted implementation",
...
def audit_fuse_source() -> dict[str, Any]:
    ...
    return {
        ...
        "mentions": mentions,
        "fuse_source_pass": FUSE_SOURCE.exists() and all(item["scoped"] for item in mentions),
        "scope": "Mounted FUSE source contains no io_uring/eBPF mounted completion path.",
    }
