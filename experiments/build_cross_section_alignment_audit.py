...
def build_report() -> dict[str, Any]:
    ...
    for name, text in texts.items():
        ...
        section_rows.append({
            ...
            "passes": all(terms_present.values()) and all(mode_present.values()),
        })
    ...
    source_anchors = {
        "abstract": find_first_line("Paper/main.tex", ["foreground SQLite p99", "13.8", "8.8", "2.7"]),
        "introduction": find_first_line("Paper/1_Introduction.tex", ["Mounted secure-storage pressure", "13.8", "8.8", "2.7"]),
        "design": find_first_line("Paper/3_Design.tex", ["SQLite", "13.8", "8.8", "2.7"]),
        "evaluation": find_first_line("Paper/4_Evaluation.tex", ["Mounted storage pressure", "13.8", "8.8", "2.7"]),
        "conclusion": find_first_line("Paper/6_Conclusion.tex", ["SQLite", "13.8", "8.8", "2.7"]),
    }
    ...
    return {
        ...
        "scope": [
            ...
            "rounded SQLite hero claim terms: 6.4, 13.8, 8.8, 2.7, deadline, background",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    for row in report["section_alignment"]:
        lines.append(
            ...
            f"`{all(row['hero_terms'].values())}` | `{all(row['mode_terms'].values())}` | `{row['passes']}` |"
        )
