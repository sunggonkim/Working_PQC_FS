...
FIGURE_SCRIPT = ROOT / "experiments" / "build_paper_spine_gate.py"
...
def build_report() -> dict[str, Any]:
    ...
    metrics = {mode: metric_row(modes[mode]) for mode in REQUIRED_MODES if mode in modes}
    ...
    exact_claim = (
        "For foreground SQLite transactions on mounted AEGIS-Q FUSE, "
        f"unthrottled secure-storage pressure raises p99 from {app.get('p99_ms'):.3f} ms "
        f"to {pressure.get('p99_ms'):.3f} ms and causes {pressure.get('deadline_misses')} "
        f"deadline miss; AEGIS-Q policy recovers p99 to {aegis.get('p99_ms'):.3f} ms "
        f"with {aegis.get('deadline_misses')} misses while retaining "
        f"{aegis.get('background_mb_s'):.3f} MB/s of background progress."
    )
    displayed_claim = (
        "Mounted secure-storage pressure raises foreground SQLite p99 from "
        f"{rounded(app.get('p99_ms', 0.0))} ms to {rounded(pressure.get('p99_ms', 0.0))} ms "
        f"with one deadline miss; AEGIS-Q recovers to {rounded(aegis.get('p99_ms', 0.0))} ms, "
        f"zero misses, and {rounded(aegis.get('background_mb_s', 0.0))} MB/s background progress."
    )
    ...
    paper_locations = {
        "abstract": find_line(PAPER / "main.tex", ["foreground SQLite p99", "13.8", "8.8", "2.7"]),
        "introduction": find_line(PAPER / "1_Introduction.tex", ["Figure~\\ref{fig:first_page_qos}", "13.8", "8.8", "2.7"]),
        "evaluation": find_line(PAPER / "4_Evaluation.tex", ["Table~\\ref{tab:qos_sqlite_recovery}", "13.8", "8.8", "2.7"]),
        "scope_boundary": find_line(PAPER / "4_Evaluation.tex", ["This is SQLite recovery", "not general AI-inference QoS"]),
    }
    ...
    checks = {
        ...
        "component_coverage_pass": all(figure_data.get("component_coverage", {}).values()),
        ...
        "aegis_removes_deadline_misses": pressure.get("deadline_misses") and aegis.get("deadline_misses") == 0,
    ...
    return {
        ...
        "artifact_path": relpath(QOS_BUNDLE),
        "raw_log_paths": {
            mode: {
                key: value
        ...
        "scope_boundary": [
            ...
            "not foreground TensorRT/AI p99 recovery",
