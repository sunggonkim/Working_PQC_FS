...
CONTRIBUTION_MAP = [
    {
        "id": "C1",
        "title": "Secure-storage format and placement boundary",
        "intro_needles": ["C1:", "Section~\\ref{sec:design_pipeline}", "Section~\\ref{sec:eval_workloads}"],
        "design_needles": ["\\label{sec:design_pipeline}", "fig:djc_state_machine"],
        "evaluation_needles": ["\\label{sec:eval_workloads}", "generation fault matrix"],
    },
    {
        "id": "C2",
        "title": "Scoped placement and workflow evidence",
        "intro_needles": ["C2:", "Section~\\ref{sec:design_uma}", "Section~\\ref{sec:eval_performance}"],
        "design_needles": ["\\label{sec:design_uma}", "elastic lane"],
        "evaluation_needles": ["\\label{sec:eval_performance}", "CPU/OpenSSL"],
    },
    {
        "id": "C3",
        "title": "Recovery/freshness evidence",
        "intro_needles": ["C3:", "Section~\\ref{sec:design_security}", "Section~\\ref{sec:eval_workloads}"],
        "design_needles": ["\\label{sec:design_security}", "TPM NV index"],
        "evaluation_needles": ["TPM replay-after-advance", "recovery matrix"],
    },
    {
        "id": "C4",
        "title": "Telemetry-to-storage QoS evidence",
        "intro_needles": ["C4:", "Section~\\ref{sec:design_uma}", "Section~\\ref{sec:eval_qos}"],
        "design_needles": ["user.pqc\\_qos\\_class", "mounted-FUSE throttling"],
        "evaluation_needles": ["\\label{sec:eval_qos}", "SQLite recovery"],
    },
]
...
def write_figure_data(bundle: dict[str, Any], result_dir: Path) -> list[dict[str, Any]]:
    ...
    json_path.write_text(json.dumps({
        "source": relpath(QOS_BUNDLE),
        "rows": rows,
        "checks": bundle.get("recovery_checks", {}),
        "component_coverage": bundle.get("component_coverage", {}),
...
def audit_paper(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    figure_values_present = all(
        f"{row['p99_ms']:.1f}" in first_page for row in rows
...
def main() -> int:
    ...
    rows = write_figure_data(bundle, args.result_dir)
